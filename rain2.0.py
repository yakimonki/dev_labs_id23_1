...
class Cloud:
    ...
    def create_drop(self):
        ...
        return Drop(drop_x, drop_y, {'angle': angle, 'speed': speed})
    ...
    def draw(self, painter):
        painter.setBrush(QColor(105, 105, 105))  # Темно-серый цвет туч
...
class RainWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.selected_cloud:
            pen = QPen(QColor(255, 255, 255, 128))  # Полупрозрачный белый контур
            pen.setWidth(3)
            painter.setPen(pen)
...
class CloudEditDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.angle_slider.setValue((self.selected_cloud.min_angle + self.selected_cloud.max_angle) // 2)
        self.angle_slider.valueChanged.connect(self.change_angle)
        layout.addWidget(QLabel("Наклон капель (градусы):"))
    ...
    def change_angle(self, value):
        self.selected_cloud.min_angle = value - 10
        self.selected_cloud.max_angle = value + 10
